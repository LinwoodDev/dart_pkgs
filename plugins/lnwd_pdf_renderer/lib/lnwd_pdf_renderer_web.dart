...
class LnwdPdfRendererWeb extends LnwdPdfRendererPlatform {
  ...
  /// Returns a [PdfDocument] containing all pages of the given pdf [data].
  @override
  Future<PdfDocument> render(Uint8List data) async {
    final document = await promiseToFuture<PdfJsDocument>(
        PdfJsLib.getDocument(data).promise);
    final pages = <PdfPage>[];
    for (int i = 0; i < document.numPages; i++) {
      final page = await promiseToFuture<PdfJsPage>(document.getPage(i + 1));
      ...
      // Convert png to Uint8List
      final bytes = base64.decode(png.substring(png.indexOf(',') + 1));
      pages.add(PdfPage(
